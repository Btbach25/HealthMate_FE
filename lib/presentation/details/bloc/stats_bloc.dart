...
class StatsBloc extends Bloc<StatsEvent, StatsState> {
  ...
  StatsBloc({required StatsRepository statsRepository, DeviceHealthCubit? deviceCubit})
  ...
  Future<void> _onFetchStatsData(
    ...
    try {
      final statsData = await _statsRepository.getStatsPageData(range: state.selectedRange);
      if (statsData.metrics.isNotEmpty) {
        emit(state.copyWith(status: StatsStatus.loaded, statsData: statsData, isFromDevice: false));
        return;
      }
    } catch (_) {}
    ...
    if (fallback != null) {
      emit(state.copyWith(status: StatsStatus.loaded, statsData: fallback, isFromDevice: true));
    } else {
      ...
      emit(state.copyWith(status: StatsStatus.loaded, statsData: StatsPageData.empty(), isFromDevice: false));
    }
  }
  ...
  Future<void> _onChangeRange(
    ...
    try {
      final statsData = await _statsRepository.getStatsPageData(range: event.range);
      if (statsData.metrics.isNotEmpty) {
        emit(state.copyWith(status: StatsStatus.loaded, statsData: statsData, isFromDevice: false));
        return;
      }
  ...
  void _onTryDeviceFallback(
    TryDeviceFallback event,
    Emitter<StatsState> emit,
  ) {
    // Bỏ qua nếu đã có data thật từ server
    final hasServerData = state.status == StatsStatus.loaded &&
        !state.isFromDevice &&
